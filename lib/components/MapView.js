...
class MapView extends React.Component {
  ...
  _rotateMap() {
    ...
    this.map.setNativeProps({ bearing });
  }
...
<<<<<<< HEAD
        ...
        this.map.setNativeProps({ initialRegion });
=======
        this.map.setNativeProps({ region, bearing });
>>>>>>> 🤖 Add bearing property for android
=======
        this.map.setNativeProps({ region });
>>>>>>> Move property assignment to its own function.
      }
    }
  ...
  setMapBoundries(coordinates = []) {
    this._runCommand('setMapBoundries', [coordinates]);
  }
